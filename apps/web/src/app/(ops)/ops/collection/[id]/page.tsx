...
function repoLaneHref(targetType: string, targetId: string) {
...
export default async function OpsCollectionDetailPage({ params, searchParams }: Props) {
  ...
  const [proposalData, repoChangeData] = await Promise.all([
    ...
    listRepoChanges({ targetType: "collection", targetId: id, limit: 5 }),
  ]);
  const relatedProposal =
    proposalData.items
      .filter((item) => item.type === "incident_followup" && item.targetType === "collection" && item.targetId === id)
  ...
  const purchaseDiagnostics = await getPurchaseDiagnostics({ targetType: "collection", targetId: id });
  ...
  const purchaseDiagTone =
    purchaseDiagnostics.status === "aligned"
      ? "border-emerald-200 bg-emerald-50 text-emerald-800"
      : purchaseDiagnostics.status === "missing_snapshot"
      ...
      <div id="governance-state" className="mt-4 rounded-2xl border border-zinc-200 bg-white p-4">
        ...
        <div className={`mt-4 rounded-xl border p-3 ${governanceToneClass(governanceState.tone)}`}>
          ...
          <div className="mt-3 flex flex-wrap gap-3 text-xs">
            ...
            {relatedRepoChange ? (
              <Link className="underline" href={repoLaneHref("collection", id)}>
                Open repo change lane
              </Link>
            ) : null}
            <Link className="underline" href={`/ops/audit?targetType=collection&targetId=${encodeURIComponent(id)}`}>
              Open audit
            </Link>
            <Link className="underline" href="/ops/monitoring?type=collection">
              Open monitoring
            </Link>
            <Link className="underline" href="#recommendations">
                ...
                <div key={rec.id} className="rounded-xl border border-zinc-200 p-3">
                      ...
                      <>
                        <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
                          ...
                          {linkedProposal ? (
                            ...
                            <Link className="text-xs underline" href={repoLaneHref("collection", id)}>
                              Open repo change lane
                            </Link>
                          ) : rec.preparedDraft ? (
                            <Link className="text-xs underline" href={`/ops/collection/${id}?draft=${rec.preparedDraft.draftId}`}>
                              Open prepared draft
                            </Link>
                          ) : (
                            <Link className="text-xs underline" href={`/ops/audit?targetType=collection&targetId=${encodeURIComponent(id)}`}>
                              Open audit
                            </Link>
                          )}
                        </div>
                  ...
                  {rec.context ? (
                    <div className="mt-3 rounded-xl bg-zinc-50 p-3 text-xs text-zinc-600">
                      ...
                      {rec.context.delta ? (
                        <p className="mt-1">
                          vs prev: CTA {fmtPts(rec.context.delta.rates.ctaRate)} · ATC {fmtPts(rec.context.delta.rates.addToCartRate)} · Purchase{" "}
                          {fmtPts(rec.context.delta.rates.purchaseRate)}
                        </p>
                      ) : (
                      ...
                      {rec.context.actionHints?.length ? <p className="mt-2">建议：{rec.context.actionHints.join("；")}</p> : null}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-zinc-200 bg-white p-6">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm font-medium text-zinc-900">Purchase diagnostics</p>
            <span className={`rounded border px-2 py-0.5 text-xs ${purchaseDiagTone}`}>{purchaseDiagnostics.status}</span>
          </div>
          <p className="mt-2 text-xs text-zinc-500">
            Window {purchaseDiagnostics.windowDays}d · until {purchaseDiagnostics.untilAt}
          </p>
          <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
            <div className="rounded-xl bg-zinc-50 p-3">
              <p className="text-xs text-zinc-500">Snapshot purchases</p>
              <p className="mt-1 font-semibold text-zinc-900">{purchaseDiagnostics.snapshotPurchaseCount}</p>
            </div>
            <div className="rounded-xl bg-zinc-50 p-3">
              <p className="text-xs text-zinc-500">Event purchases</p>
              <p className="mt-1 font-semibold text-zinc-900">{purchaseDiagnostics.eventPurchaseCount}</p>
            </div>
            <div className="rounded-xl bg-zinc-50 p-3">
              <p className="text-xs text-zinc-500">Gap</p>
              <p className={`mt-1 font-semibold ${purchaseDiagnostics.gap === 0 ? "text-emerald-700" : "text-rose-700"}`}>
                {purchaseDiagnostics.gap > 0 ? "+" : ""}
                {purchaseDiagnostics.gap}
              </p>
            </div>
          </div>
          {purchaseDiagnostics.latestSnapshot ? (
            <p className="mt-3 text-xs text-zinc-500">
              Snapshot: {purchaseDiagnostics.latestSnapshot.capturedAt} · {purchaseDiagnostics.latestSnapshot.source}
              {purchaseDiagnostics.latestSnapshot.contentRef ? ` · ${purchaseDiagnostics.latestSnapshot.contentRef}` : ""}
            </p>
          ) : (
            <p className="mt-3 text-xs text-amber-700">No snapshot yet. Run a snapshot before trusting purchase deltas.</p>
          )}
          <div className="mt-4 space-y-2">
            {purchaseDiagnostics.bySource.length ? (
              purchaseDiagnostics.bySource.map((row) => (
                <div key={row.source} className="flex items-center justify-between rounded-xl border border-zinc-200 px-3 py-2 text-sm">
                  <div>
                    <p className="font-medium text-zinc-900">{row.source}</p>
                    <p className="text-xs text-zinc-500">{row.latestAt ? `latest ${row.latestAt}` : "no timestamp"}</p>
                  </div>
                  <p className="font-semibold text-zinc-900">{row.count}</p>
                </div>
              ))
            ) : (
              <p className="text-sm text-zinc-600">No purchase events in the current window.</p>
            )}
          </div>
        </div>
        <div className="rounded-2xl border border-zinc-200 bg-white p-6">
