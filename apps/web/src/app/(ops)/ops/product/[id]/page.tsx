...
export default async function OpsProductDetailPage({ params, searchParams }: Props) {
  ...
  const [proposalData, repoChangeData] = await Promise.all([
    ...
    listRepoChanges({ targetType: "product", targetId: id, limit: 5 }),
  ]);
  const relatedProposal =
    proposalData.items
      .filter((item) => item.type === "incident_followup" && item.targetType === "product" && item.targetId === id)
  ...
  const purchaseDiagnostics = await getPurchaseDiagnostics({ targetType: "product", targetId: id });
      ...
      <div id="governance-state" className="mt-4 rounded-2xl border border-zinc-200 bg-white p-4">
        ...
        <div className={`mt-4 rounded-xl border p-3 ${governanceToneClass(governanceState.tone)}`}>
          ...
          <div className="mt-3 flex flex-wrap gap-3 text-xs">
            ...
            {relatedRepoChange ? (
              <Link className="underline" href={repoLaneHref("product", id)}>
                Open repo change lane
              </Link>
            ) : null}
            <Link className="underline" href={`/ops/audit?targetType=product&targetId=${encodeURIComponent(id)}`}>
              Open audit
            </Link>
            <Link className="underline" href="/ops/monitoring?type=product">
              Open monitoring
            </Link>
                      ...
                      <>
                        <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
                          ...
                          {linkedProposal ? (
                            ...
                            <Link className="text-xs underline" href={repoLaneHref("product", id)}>
                              Open repo change lane
                            </Link>
                          ) : rec.preparedDraft ? (
                            <Link className="text-xs underline" href={`/ops/product/${id}?draft=${rec.preparedDraft.draftId}`}>
                              Open prepared draft
                            </Link>
                          ) : (
                            <Link className="text-xs underline" href={`/ops/audit?targetType=product&targetId=${encodeURIComponent(id)}`}>
                              Open audit
                            </Link>
                          )}
                        </div>
